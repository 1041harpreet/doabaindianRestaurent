...
Widget makePayment(cartprovider, checkoutprovider, parentcontext, double total, tax, itemlist, notificationprovider, context) {
  var sub = total - tax;
      ...
      transactions: [
        {
          "amount": {
            "total": total,
            "currency": "USD",
            "details": {"subtotal": subtotal, "tax": tax, "shipping_discount": 0}
          },
        ...
        if (params['status'] == 'success') {
          ...
          SchedulerBinding.instance.addPostFrameCallback((timeStamp) async {
            Navigator.of(context).pushAndRemoveUntil(
                ...
                (route) => false);
            showSuccessToast(context: parentcontext, message: "Payment Successfully Completed,Check your email.");
            if (params['status'] == "success") {
              print('success');
              await checkoutprovider.getAdminToken();
              //used to send email to user
              MailService().userMail(email, fullname, checkoutprovider.date, total, params['paymentId']);
              //used to send mail to admin
              MailService().adminMail(fullname, checkoutprovider.date, total, params['paymentId']);
              //used to send data to admin
              checkoutprovider.sendToAdmin(
                  email, params['paymentId'], checkoutprovider, roundDouble(cartprovider.total, 2), cartprovider.tax, cartprovider.orderItem);
              //used to send notification to user
              NotificationController().createNewNotification(
                  "Hey ${fullname}! Your order is confirmed", "Your order on ${email} of total ${cartprovider.total} is placed. ", usertoken);
              //used to send notification to admin
              NotificationController().createNewNotification(
                  'Hey ! Order From ${fullname}', "New order on ${email} of total ${cartprovider.total} is Placed.", checkoutprovider.adminToken);

              //add on to notification
              notificationprovider.addToNotification(params['paymentId'], email, true, tax, total, checkoutprovider.date);
            }
          });
        }
      },
      ...
      onCancel: (params) {
        ...
        SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
          Navigator.of(context)
              .pushAndRemoveUntil(MaterialPageRoute(builder: (context) => PaymentFailedScreen(orderID: params['token'])), (route) => false);
          showErrorToast(context: parentcontext, message: "Payment Failed");
          NotificationController().createNewNotification('Hey ! Your Payment is failed', "Please try Again .!", usertoken);
        });
